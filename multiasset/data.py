...
def _load_fx_curve_artifact():
    ...
    try:
        ...
        try:
            ...
            # The file exists but neither loader can unpickle it, and the
            # BlockManager-integrity repair above (see pickle_repair.py) either
            # doesn't apply or didn't help -- this is NOT a missing-file
            # problem (that's handled above), it's the pickle's on-disk format
            # being incompatible with the running pandas/pickle version in some
            # other way. Re-raising as FileNotFoundError (as this used to) hid
            # that distinction and sent whoever hit it looking for a missing
            # file instead of a stale/incompatible artifact that needs to be
            # regenerated by its producer.
            raise RuntimeError(
                ...
                f"not a missing file -- the .pkl needs to be regenerated by "
                f"whatever produces it (outside this repo; DIR_INPUT is the "
                f"shared artifact store), not repaired here."
            ) from exc_raw
