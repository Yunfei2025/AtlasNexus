...
def _load_fx_curve_artifact():
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"fxcurve_ts.pkl not found in {DIR_INPUT}")
    try:
        ...
        try:
            ...
            # The file exists but neither loader can unpickle it -- this is
            # NOT a missing-file problem (that's handled above), it's the
            # pickle's on-disk format being incompatible with the running
            # pandas/pickle version. Observed cause: pandas >= 3.0's
            # DataFrame.__setstate__ verifies BlockManager integrity more
            # strictly than older pandas did, so a DataFrame pickled by an
            # older pandas (or written with duplicate/misaligned columns)
            # raises AssertionError on unpickle here instead of loading with
            # silently wrong data. Re-raising as FileNotFoundError (as this
            # used to) hid that distinction and sent whoever hit it looking
            # for a missing file instead of a stale/incompatible artifact
            # that needs to be regenerated by its producer.
            raise RuntimeError(
                f"fxcurve_ts.pkl at {file_path} exists but failed to unpickle "
                f"under pandas {pd.__version__}: {exc_pandas!r}. This is a "
                f"pandas-version/format incompatibility in the pickle itself "
                f"(e.g. a BlockManager integrity error), not a missing file -- "
                f"the .pkl needs to be regenerated by whatever produces it "
                f"(outside this repo; DIR_INPUT is the shared artifact store), "
                f"not repaired here."
            ) from exc_raw
