...
def _load_fx_curve_artifact(input_dir: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            # See multiasset/data.py::_load_fx_curve_artifact for why this is
            # raised as RuntimeError, not FileNotFoundError -- the file is
            # present but its on-disk pickle format is incompatible with the
            # running pandas (observed: pandas >= 3.0's stricter BlockManager
            # integrity check on unpickle), which is a different problem
            # from "file missing" and needs a different fix (regenerate the
            # artifact, don't look for a missing path).
            raise RuntimeError(
                ...
                f"not a missing file -- it needs to be regenerated by "
                f"whatever produces it, not repaired here."
            ) from exc_raw
