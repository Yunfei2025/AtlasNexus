...
def pkl_lock(file_path: str, timeout: float = 60.0, poll: float = 0.05):
    """Cross-platform advisory lock for a read-modify-write cycle on ``file_path``.

    The EOD pipeline and the intraday scheduler both call ``refresh_otr_ofr_rv_spreads``
    (and other refreshers) against the *same* pickle with no coordination: each does
    ``loadPKL`` -> merge in memory -> ``updatePKL(..., rewrite=True)``. Without a lock
    spanning that whole cycle, two overlapping runs can each read a clean snapshot,
    merge independently, and the second writer's blind ``rewrite=True`` dump clobbers
    the first writer's update -- observed in production as duplicated index dates in
    TBond-spds.pkl (two structurally different rows for the same date, one per writer's
    stale in-memory snapshot). ``open(..., 'x')`` (O_CREAT|O_EXCL) is atomic on POSIX
    and Windows alike, so a bare lockfile needs no extra dependency (e.g. filelock).
    """
    ...
    while fd is None:
        try:
            ...
            if time.monotonic() >= deadline:
                # Stale lock from a crashed process -- proceed rather than
                # deadlock forever; a lost race here just reproduces the
                # pre-existing (already-tolerated) unlocked behavior.
                break
            time.sleep(poll)
    try:
        yield
...
def updatePKL(dictn, file_path, rewrite=False):
    if rewrite:
        # Callers doing their own external loadPKL -> merge -> updatePKL(rewrite=True)
        # cycle (e.g. curves/refreshers/otr_ofr_rv.py) must hold pkl_lock() around
        # that whole cycle themselves -- locking only this write would still leave
        # the read-modify-write race that corrupts the file (see pkl_lock's docstring).
        with open(file_path, 'wb') as file:
            pickle.dump(dictn, file, protocol=pickle.HIGHEST_PROTOCOL)
        return dictn
    else:
        with pkl_lock(file_path):
            if os.path.exists(file_path):
