...
class WindConfig:
    ...
    MTNCV_IDS = "M1000532,M1000533,M1000534,M1000535,M1000536"
    LGBCV_IDS = "M1008638,M1008640,M1008641,M1008643,L5689539"

    CDBCVD_LIST = CDBCVD_IDS.split(',')
    CGBCV_LIST = CGBCV_IDS.split(',')
    CPCV_LIST  = CPCV_IDS.split(',')
    MTNCV_LIST = MTNCV_IDS.split(',')
    LGBCV_LIST = LGBCV_IDS.split(',')
    ...
    CV_LIST = [
        ...
        '中债中短期票据到期收益率(AAA):1年','中债中短期票据到期收益率(AAA):2年','中债中短期票据到期收益率(AAA):3年',
        '中债中短期票据到期收益率(AAA):4年','中债中短期票据到期收益率(AAA):5年',
        '中国:地方政府债到期收益率(AAA):1年','中国:地方政府债到期收益率(AAA):3年','中国:地方政府债到期收益率(AAA):5年',
        '中国:地方政府债到期收益率(AAA):10年','中国:地方政府债到期收益率(AAA):30年',
    ]

    CV_ID_MAP = dict(zip(CDBCVD_LIST + CGBCV_LIST + CPCV_LIST + MTNCV_LIST + LGBCV_LIST, CV_LIST))

    KTID = {'CGB': {}, 'CDB': {}, 'ICP': {},'MTN': {},'LGB': {}}
    for t in ['S0059741','S0059742','S0059743','S0059744', 'S0059745', 'S0059746', 'M0057946', 'S0059747', 'S0059748', 'S0059749', 'S0059751', 'S0059752']:
    ...
    for t in MTNCV_LIST:
        KTID['MTN'][t] = CV_ID_MAP[t]
    for t in LGBCV_LIST:
        KTID['LGB'][t] = CV_ID_MAP[t]
