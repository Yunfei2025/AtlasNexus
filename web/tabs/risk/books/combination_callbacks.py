...
def register_combination_callbacks(app):
    ...
    def _render_combination(alpha_capital_mm, alpha_weight_pct):
        ...
        # --- Frontier: combined Sharpe across every weight ---
        sweep = result['sweep']
        frontier = go.Figure()
        ...
        frontier.update_layout(
            ...
            height=260, margin={'l': 55, 'r': 20, 't': 30, 'b': 45}, showlegend=False,
        )

        # --- Equity curves, all rebased to cumulative % return ---
        curves = go.Figure()
        for key, color, label in (
            ('beta', THEME['accent'], 'Beta book'),
            ('alpha', THEME['warning'], 'Alpha book'),
            ('combined', THEME['success'], f'Combined ({(1 - w) * 100:.0f}/{w * 100:.0f})'),
        ):
            ...
            curves.add_trace(go.Scatter(
                ...
                line={'color': color, 'width': 2.2 if key == 'combined' else 1.3},
            ))
        curves.update_layout(
            ...
            template='plotly_dark', paper_bgcolor=THEME['bg_card'], plot_bgcolor=THEME['bg_card'],
            height=280, margin={'l': 55, 'r': 20, 't': 30, 'b': 35},
        ...
        body = html.Div([
            html.Div([
                ...
                _metric_card('Combined', combined, THEME['success']),
                reco,
            ...
            html.Div([
                ...
                html.Div([dcc.Graph(figure=curves, config={'displayModeBar': False})],
                         style={'flex': '1', 'minWidth': '320px'}),
