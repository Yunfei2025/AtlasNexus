"""Bond futures dashboard entry point."""
...
from web.core.styles import app_color, color_mode
from web.core.funcs import candlestick_trace, queryPriceVolumeData
        ...
        shapes=[
            {
                ...
                "y1": mean,
                "type": "line",
                "line": {"dash": "dash", "color": "#BD9391", "width": 3},
            },
            {
                "xref": "x",
                "yref": "y",
                ...
                "y1": mean - std,
                "type": "line",
                "line": {"dash": "dot", "color": "#BD9391", "width": 3},
            },
            {
                "xref": "x",
                "yref": "y",
...
def gen_vol_histogram(interval, date, futures, csinterval):
    try:
        ...
        price_vol = data_dict["vol_prof"]
        bin_val = (
            np.array(price_vol.values),
            np.array(price_vol.index),
        )

        price_vol_bf_last_min = data_dict["bf_lst_vol"]
        bin_bf_last_min_val = (
            np.array(price_vol_bf_last_min.values),
            np.array(price_vol_bf_last_min.index),
        )

        price_vol_last_min = data_dict["vol_last_min"]
        bin_last_min_val = (
            np.array(price_vol_last_min.values),
            np.array(price_vol_last_min.index),
        )
    ...
    trace = dict(
        type="bar",
        x=bin_bf_last_min_val[0],
        y=bin_bf_last_min_val[1],
    ...
    trace_last_min = dict(
        type="bar",
        x=bin_last_min_val[0],
        y=bin_last_min_val[1],
        marker={"color": color_mode[np.sign(value)]},
        showlegend=False,
        hoverinfo="x+y",
        orientation="h",
    )
        ...
        shapes=[
            {
                "xref": "x",
                "yref": "y",
                "x1": int(max(bin_val_max, y_val_max)) + 0.5,
                "x0": 0,
                "y0": mean,
                "y1": mean,
                "type": "line",
                "line": {"dash": "dash", "color": "#BD9391", "width": 5},
            },
            {
                "xref": "x",
                "yref": "y",
            ...
            },
            {
                ...
                "y1": median_val,
                "type": "line",
                "line": {"dash": "dot", "color": "#2E5266", "width": 5},
            },
        ],
    )
