...
\subsection{30Y CGB On/Off-the-Run Relative Value within \texttt{TBondCurve}}
\label{sec:pairs-tbondcurve-otr-ofr}

The 30Y China Government Bond (CGB) on/off-the-run (OTR/OFR) trade is represented as
a \texttt{TBondCurve} sub-strategy, rather than as an extrapolation of the fitted
1--10Y model curve.  Its economic object is the 30Y liquidity premium: the current,
most liquid eligible 30Y CGB (OTR) is compared with a liquid prior 30Y issue (OFR).
...
\paragraph{OTR rolls and new-issue cold start.}
...
1--10Y range.  It shares the \texttt{TBondCurve} data, risk, candidate, allocation,
and backtest plumbing, but has an explicit \texttt{otr\_ofr} signal variant so that
pair selection, spread sign, DV01 hedge ratio, roll handling, history sufficiency,
and transaction-cost assumptions remain distinguishable from the model-curve
residual variant.
...
Trend candidates receive a direction-specific carry--momentum score. Let $d$ denote
an executable BUY or SELL spread direction, $C_{i,d}$ the expected carry, roll,
financing, and borrow contribution scaled by return volatility, and $M_{i,d}$ the
medium-horizon economically oriented trend contribution scaled by the same risk
measure. Within comparable spread families, the score is formed from cross-sectional
standardized factors,
\begin{equation}
R_{i,d}=w_C z(C_{i,d})+w_M z(M_{i,d}), \qquad w_C+w_M=1.
\end{equation}
The initial specification uses $w_C=w_M=0.5$. For each spread, the implementation
retains only the feasible direction with the larger expected edge. It does not treat
the lowest non-negative candidate score as an automatic short: direction is an
economic property of the position, while the score measures its strength.

Correlation, capacity, DV01, and margin constraints are applied after this
style-specific ranking. The dated allocation snapshot retains the source-data
as-of timestamp, factor values, selected direction, score, and allocation. Neither
the carry--momentum score nor a subsequent change in rank may act as an intramonth
entry, exit, or reversal gate.
...
\paragraph{Current versus target trend implementation.} The current individual trend
engine enters immediately when its forward-filled absolute DC state is positive or
negative, exits on an opposite state after \texttt{min\_hold}, and applies a trailing
stop using the current rolling standard deviation of daily changes. Its adaptive
threshold is recalculated by month as a base threshold times a bounded ratio of the
latest robust scale to its historical median. The family-level multiplier/floor/cap,
frozen trailing-scale, and stateful month-to-month trade lifecycle described below are
target controls for the planned walk-forward implementation, not all current runtime
behavior.
...
\paragraph{Trend-following pullback timing research.} For a trend candidate selected
by the monthly carry--momentum rank, a separate continuation-timing variant may enter
earlier than the DC-continuation baseline without converting the rank into an entry
gate. First orient the spread into economic-PnL direction and estimate fast and slow
local slopes, $\beta^{\mathrm{fast}}_t$ and $\beta^{\mathrm{slow}}_t$. Define a short
horizon acceleration as $a_t=\beta^{\mathrm{fast}}_t-
\beta^{\mathrm{fast}}_{t-h}$. A long entry requires positive fast and slow slopes,
positive acceleration, a mild pullback below the fast trend estimate while remaining
above the slow estimate, and a reconfirmation back through the fast estimate. The
short rule is sign-symmetric. The residual z-score is restricted to
$|z_t|\le z_{\max}$ solely to avoid buying or selling an overextended move; it is not
a directional trigger. Exits remain an opposite confirmed DC event, a
volatility-scaled trailing stop, or a persistent slope reversal.

This is a named \emph{trend-following pullback} research specification, distinct from
the DC-continuation production baseline. It can be promoted only through anchored,
family-level walk-forward tests using next-bar execution, explicit costs, turnover
and capacity limits, and a final held-out period.
...
its own MR or DC trend trade state through month boundaries; a changed style or removed
...
while its DC state is downward and its moving-average $z$-score remains positive, and
sells while its state is upward and $z$ remains negative, is a counter-trend
turning-point strategy rather than continuation trend following. It can be researched
as a separate, explicitly labeled model, but it should not replace DC continuation by
assumption: it may enter before a persistent move has ended. A safer research sequence
compares (i) DC continuation, (ii) same-direction DC with a bounded z-score pullback,
